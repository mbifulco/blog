...
**EDIT: ** Hi, Mike from The Future™️ here - I've edited this paragraph to reflect the _actual_ truth... I had confused `-1` with `false` from my days as a BASIC programmer, where we'd sometimes create infinite loops with `while (-1)`... but even that means "while `true`"!
