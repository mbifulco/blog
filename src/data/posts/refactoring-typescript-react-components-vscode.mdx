---
title: 'Refactoring TypeScript React components in VS Code for fun and profit'
...
coverImagePublicId: 'blog/refactoring-typescript-react-components-vscode/cover'
...
You've probably noticed that there is exactly 0 type-checking going on in this code. I've been writing react with good ol' plain JavaScript for so long that creating components like this is reflexive for me. I'm not even thinking about it.

Eventually during implementation I get to the point where adding some complecity to the component requires me to add some type-checking. At that point, I usually refactor the component to lambda syntax, and add a type annotation to the component:

```tsx
import Footer from "@components/Footer";
import Header from "@components/Header";

const function PageLayout = ({ children }) => {
  return (
    <>
      <section className="min-h-[100vh] pb-8 sm:pb-12 lg:pb-12">
        <Header />
...
const function PageLayout = ({ children }: { children: React.ReactNode }) => {
  return (
    <>
      <section className="min-h-[100vh] pb-8 sm:pb-12 lg:pb-12">
        <Header />
...
Taking it a step further, I'll give the function _itself_ a type signature, using `React.FC` and generics to define the props:

```tsx
import Footer from "@components/Footer";
import Header from "@components/Header";

const function PageLayout:React.FC<{children: React.ReactNode}> = ({ children }) => {
  return (
    <>
      <section className="min-h-[100vh] pb-8 sm:pb-12 lg:pb-12">
        <Header />
...
Up to this point, I don't really mind the way this component was written in any of the previous refactors. It's clearly getting more complex, but I can still understand what's going on. But what if I wanted to add a `variant` prop to the component? I'd have to add a type annotation to the function signature, and then add a type annotation to the `title` prop:

```tsx
import Footer from "@components/Footer";
import Header from "@components/Header";

const function PageLayout:React.FC<{children: React.ReactNode, variant: "dark" | "light" | "black" | "tan"}> = ({ children, variant }) => {
  return (
    <>
      <section className="min-h-[100vh] pb-8 sm:pb-12 lg:pb-12">
...
const function PageLayout:PageLayout = ({ children, title }) => {
  return (
    <>
      <section className="min-h-[100vh] pb-8 sm:pb-12 lg:pb-12">
        <Header variant={variant}/>
...
I can even add a JSDoc comment to the type definition to add some documentation:
...
Theo from t3.gg has a great video that sums up my learning strategy for TypeScript pretty well: [Don't Learn Typescript](https://www.youtube.com/watch?v=kRiD6ZpAN_o). I highly recommend watching it, or putting it in your queue for your next coffee break.
