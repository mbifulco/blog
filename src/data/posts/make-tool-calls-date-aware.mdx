---
title: 'Give your model a clock: date-aware tool calls for AI features'
date: 08-04-2026
...
tags: [ai, dev, typescript, tutorial]
...
```plaintext
🤖 I couldn't find any conversations about German Smear.
```

To the salesperson reading that, this looks like a data problem. Maybe the call never got logged. Maybe the customer used different words. What it does _not_ look like is a bug, so until someone notices, nobody files anything. You can ship this and never hear about it.
...
## A date on its own isn't enough
...
**Day of week.** "Last weekend" asked on a Monday and "last weekend" asked on a Friday are the same range. Asked on a _Sunday_, they're probably not - the weekend the user means is very likely the one currently happening. The model can't reason about any of that unless it knows which day it is, and asking it to derive Tuesday from `2026-08-04` is asking for trouble.
...
## Don't make the model do the date math
...
export const buildTemporalContext = (
  ...
  // startOfHour, not the raw timestamp - more on why in the caching section
  ...
  const lastWeekendStart = subWeeks(
    ...
    // if it's currently Sat or Sun, that weekend is still happening,
    // so "last weekend" means the one before it
...
Locale: ${locale.code} - weeks start on ${format(thisWeek, 'EEEE')}

Pre-resolved date ranges. Use these rather than calculating dates yourself:
- today: ${iso(now)}
- yesterday: ${iso(subDays(now, 1))}
- last weekend: ${iso(lastWeekendStart)} to ${iso(addDays(lastWeekendStart, 1))}
- this week: ${iso(thisWeek)} to ${iso(now)}
- last week: ${iso(lastWeek)} to ${iso(subDays(thisWeek, 1))}
- this month: ${iso(startOfMonth(now))} to ${iso(now)}
- last month: ${iso(startOfMonth(subMonths(now, 1)))} to ${iso(endOfMonth(subMonths(now, 1)))}`;
};
```

Now the model isn't calculating anything. It's looking things up, which is a task it's actually good at.

## Design your tools so dates resolve deterministically
...
const searchConversations = {
  ...
  description:
    ...
    'period in words ("last weekend", "the past week"), pass `relative_range` ' +
...
Two things to call out here, because both matter more than they look:
...
```ts
import { TZDate } from '@date-fns/tz';
import { format, subYears, type Locale } from 'date-fns';
import { enUS } from 'date-fns/locale';
...
## Save money with careful prompt caching

This is the part I wish someone had told me, because I shipped the bug and only caught it while looking at a bill.

**Prompt caching** is how you avoid paying full price to reprocess the same system prompt on every single request. Every major provider offers some version of it, and the savings are substantial - cached tokens typically cost around a tenth of regular input tokens.

The catch is **how** caching works: it's a _prefix match_. The provider hashes your prompt from the first character you send, and reuses the cache only up to the first byte that differs. Change one character near the top, and everything after it is invalidated and reprocessed at full price.
...
That timestamp changes every millisecond, and it sits _above_ everything else. Your cache hit rate is exactly zero, permanently, and nothing in your app will tell you. No error, no warning - just an invoice that's larger than you expected.

Two fixes, and you want both:

**1. Round the timestamp.** Ask yourself what precision you actually need. For "last weekend," hourly is plenty:
...
If your app can tolerate day-level precision, round to the day and your cache survives until midnight.

**2. Put the volatile bit last.** Your stable instructions go first, your cache breakpoint goes after them, and anything that changes per-request goes _below_ that line:
...
Now your expensive prefix caches cleanly and only the small temporal block gets reprocessed. Exact syntax varies by provider, but the principle - stable content first, volatile content after the breakpoint - holds everywhere.

**Verify it worked.** Every provider reports cache reads in the response's usage object. Fire the same request twice and look at the cached-token count on the second one. If it's zero, something above your breakpoint is still changing. That number is the only way you'll ever know, so check it deliberately.

## Close the loop: tell the user what you searched
...
> Searching conversations from Saturday, August 1 through Sunday, August 2... I found one call about German Smear, from Dana Whitfield.
...
It also handles the genuinely ambiguous cases gracefully. When somebody asks about "last weekend" on a Sunday afternoon, there's no correct answer for your code to compute - but showing your work lets the person who _does_ know just say "no, the weekend before."
...
And keep an eye on that cache hit rate while you do it.
