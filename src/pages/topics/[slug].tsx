...
const TopicPage: NextPage<TopicPageProps> = ({ topic }) => {
  // Combine and sort all content by date (excluding articles since they don't have show pages)
  const allContent = [
          ...
          <div className="mt-4 flex flex-wrap gap-2">
            {topic.tags.map((tag) => (
              <Link
                key={tag}
                href={`/tags/${tag}`}
                className="rounded-full bg-gray-100 px-3 py-1 text-sm text-gray-700 transition-colors hover:bg-pink-100 hover:text-pink-700"
              >
                #{tag}
              </Link>
            ))}
