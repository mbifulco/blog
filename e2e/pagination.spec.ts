...
test.describe('Pagination', () => {
  test.describe('Home page pagination', () => {
    ...
    test('should navigate to page 2 from home page', async ({ page }) => {
      ...
      const page2Link = page.locator('[role="navigation"][aria-label="pagination"] a[href="/page/2"]').filter({ hasText: '2' });
      if (await page2Link.count() > 0) {
        ...
        const page1Link = page.locator('[role="navigation"][aria-label="pagination"] a[href="/"]');
    ...
    test('should handle invalid page numbers with redirects', async ({ page }) => {
  ...
  test.describe('Newsletter page pagination', () => {
    ...
    test('should navigate to newsletter page 2', async ({ page }) => {
      ...
      // Check if page 2 link exists (numbered page link, not next button)
      const page2Link = page.locator('[role="navigation"][aria-label="pagination"] a[href="/newsletter/page/2"]').filter({ hasText: '2' });
      if (await page2Link.count() > 0) {
        await page2Link.click();
        ...
        // Should show either a previous button or page 1 link on page 2
        const prevLink = page.locator('a[aria-label="Go to previous page"]');
        const page1Link = page.locator('[role="navigation"][aria-label="pagination"] a[href="/newsletter"]');
    ...
    test('should navigate back to newsletter home from page 2', async ({ page }) => {
    ...
    test('should handle invalid newsletter page numbers with redirects', async ({ page }) => {
      // Test invalid page number
  ...
  test.describe('Pagination SEO', () => {
    ...
    test('should redirect /newsletter/page/1 to newsletter', async ({ page }) => {
  ...
  test.describe('Pagination performance', () => {
    ...
    test('should prefetch pagination links', async ({ page }) => {
      ...
      const nextLink = page.locator('[role="navigation"][aria-label="pagination"] a[href="/page/2"]').filter({ hasText: '2' });
      if (await nextLink.count() > 0) {
        await expect(nextLink).toBeVisible();
